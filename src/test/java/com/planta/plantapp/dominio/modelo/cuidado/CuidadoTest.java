...
class CuidadoTest {
    ...
    @Test
    @DisplayName("Equals con objeto de diferente clase retorna false")
    void testEqualsConDiferenteClase() {
        // Arrange
        cuidado = new Cuidado(TipoCuidado.RIEGO, "Riego", Integer.valueOf(3));

        // Act & Assert
        assertNotEquals("No soy un Cuidado", cuidado);
    }
    ...
    void testProgramarProximoAutomaticoEnConstructor() {
        ...
        assertTrue(!cuidado.getFechaAplicacion().isBefore(antes));
        assertTrue(!cuidado.getFechaAplicacion().isAfter(despues));
    }
